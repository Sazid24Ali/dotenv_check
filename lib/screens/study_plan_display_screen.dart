...
class _StudyPlanDisplayScreenState extends State<StudyPlanDisplayScreen> {
  ...
  void _recalculateSessionStartTimesForDay(DateTime day) {
    ...
    final firstSessionOfDayInOriginalPlan =
        widget.plan.sessions.firstWhereOrNull(
      (s) =>
          s.scheduledDate != null &&
          DateTime(
                s.scheduledDate!.year,
    ...
    for (int i = 0; i < sessionsOnThisDay.length; i++) {
      ...
      if (globalIndex != -1) {
        _sessions[globalIndex] = _sessions[globalIndex].copyWith(
          scheduledStartTime:
              currentDailyRunningTime.toLocal().toString().substring(11, 16),
        );
  ...
  void _moveSessionDown(int currentIndex, List<StudySession> sessionsForDay) {
    ...
    int targetGlobalIndexForSearch = globalIndexToMoveStart +
    ...
    if (targetGlobalIndexForSearch < _sessions.length &&
      ...
      setState(() {
        ...
        int insertAfterTargetIndex = newGlobalTargetIndex +
  ...
  Widget build(BuildContext context) {
    ...
    double averageMinutesPerDayInPlan = totalDaysInPlan > 0
        ? _sessions.fold(
            ...
            totalDaysInPlan
        : 0;
    String averageHoursPerDayInPlan =
        (averageMinutesPerDayInPlan / 60).toStringAsFixed(1);
                              ...
                              children: [
                                ...
                                if (!session.isBreak && !session.isRevision)
                                  IconButton(
                                    ...
                                    onPressed: sessionsForDay.indexOf(session) >
                                            0
                                        ? () => _moveSessionUp(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          if (session.topic != null) ...[
                                            ...
                                            if (session.topic!.timeReasoning
                                                .isNotEmpty)
                                              Text(
                                                'Reasoning: ${session.topic!.timeReasoning}',
                                              ),
                                            if (session
                                                .topic!.resources.isNotEmpty)
                                              Text(
